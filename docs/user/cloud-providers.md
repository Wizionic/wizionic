...
- **TTS / STT** — only when the *model id* is a speech deployment (Whisper, Kokoro, `tts-1`). xAI almost never lists those as models. Mic and Speak use the provider’s `/v1/tts` and `/v1/stt` APIs (and the modality-default dropdowns), not a checkbox on `grok-4.6`.
