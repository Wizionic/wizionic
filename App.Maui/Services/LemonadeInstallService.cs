...
public sealed class LemonadeInstallService : ILemonadeInstallService
{
    ...
    public async Task<LemonadeInstallResult> ConfigureAppAsync(
        HttpClient http,
        IKeyStore keyStore,
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            return LemonadeInstallResult.Ok(
                $"Wizionic configured for Lemonade at {DefaultBaseUrl}" +
                (chat is not null ? $" (selected {chat.Name})." : "."));
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "[Lemonade] Configure Wizionic failed");
            return LemonadeInstallResult.Fail(
    ...
    private async Task<(bool Ok, string Detail)> RunElevatedShellAsync(
        ...
        int timeoutMs)
    {
        var scriptPath = Path.Combine(Path.GetTempPath(), $"app-lemonade-{Guid.NewGuid():N}.sh");
