...
/// force all target=_blank / window.open into Wizionic tabs,
...
public sealed class BrowserWebViewPlatformService
{
    ...
    /// never creates an OS/Edge popup. Host opens a Wizionic tab via postMessage.
    /// </summary>
    private const string DocumentCreatedScript =
        """
        (() => {
          if (window.__appBrowserHooks) return;
          window.__appBrowserHooks = true;
          ...
          const makeStub = (initialHref) => {
            ...
            const loc = {
              ...
              set href(v) {
                href = String(v || '');
                if (!isBlank(href)) openInApp(href);
              },
          ...
          const rewireAnchor = (a) => {
            try {
              ...
              a.setAttribute('data-app-target', t);
    ...
    /// Handled=true alone cancels the engine popup; we open a Wizionic tab when Uri is known.
    /// </summary>
    ...
    private void OpenUrlInAppOrExternal(string url)
    {
        ...
        // Always keep the user inside Wizionic for new-window / target=_blank requests.
        // (Browser settings "External browser" still applies to the explicit toolbar button.)
        MainThread.BeginInvokeOnMainThread(() =>
        {
            Console.WriteLine($"[Browser] open in Wizionic tab: {url}");
    ...
    private void OnDownloadStarting(object? sender, Microsoft.Web.WebView2.Core.CoreWebView2DownloadStartingEventArgs args)
    {
        ...
        MainThread.BeginInvokeOnMainThread(async () =>
        {
            try
            {
                ...
                // Hide WebView2's default download UI — we surface progress in the Wizionic toolbar.
                args.Handled = true;
