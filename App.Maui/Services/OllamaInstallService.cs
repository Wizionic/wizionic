...
public sealed class OllamaInstallService : IOllamaInstallService
{
    ...
    public async Task<OllamaInstallResult> ConfigureAppAsync(
        HttpClient http,
        IKeyStore keyStore,
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            return OllamaInstallResult.Ok(
                $"Wizionic configured for Ollama at {DefaultBaseUrl}" +
                (first is not null ? $" (selected {first.Name})." : "."));
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "[Ollama] Configure Wizionic failed");
    ...
    private async Task<(bool Ok, string Detail)> RunElevatedShellAsync(
        ...
        int timeoutMs)
    {
        var scriptPath = Path.Combine(Path.GetTempPath(), $"app-ollama-{Guid.NewGuid():N}.sh");
