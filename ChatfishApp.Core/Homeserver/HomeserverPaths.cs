...
/// <summary>
/// Stable filesystem layout for a Windows / Linux Chatfish Home Server install.
...
public static class HomeserverPaths
{
    ...
    public const string SystemdUnitName = "chatfish-homeserver";
    ...
    /// Windows: %ProgramData%\Chatfish\Homeserver
    /// Linux:   ~/.local/share/Chatfish/Homeserver  (user-local; works without root)
    /// </summary>
    public static string RootDirectory
    {
        get
        {
            if (OperatingSystem.IsLinux())
            {
                ...
                return Path.Combine(local, "Chatfish", "Homeserver");
            }
    ...
    /// <summary>Published host entrypoint (ChatfishApp.exe on Windows, ChatfishApp on Linux).</summary>
    public static string HostExecutablePath =>
        Path.Combine(AppDirectory, OperatingSystem.IsWindows() ? "ChatfishApp.exe" : "ChatfishApp");
    ...
    public static string LinuxAutostartDesktopPath
    {
        get
        {
            ...
            return Path.Combine(config, "autostart", "chatfish-homeserver.desktop");
        }
    }
