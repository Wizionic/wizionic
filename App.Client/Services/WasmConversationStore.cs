...
public class WasmConversationStore : IConversationStore
{
    ...
    public async Task<List<SyncManifestEntry>> LoadManifestEntriesAsync(bool backfillMissingFingerprints = false, CancellationToken ct = default)
    {
        ...
        foreach (var m in metas)
        {
            ...
            if (!deletedAtTicks.HasValue)
            {
                ...
                var computed = SyncFingerprint.ForConversation(m.id, title, messages, m.isPasswordProtected == true, m.protectionChangedTicks ?? 0);
    ...
    public async Task SetConversationTitleAsync(string id, string title, CancellationToken ct = default)
    {
        ...
        var contentFingerprint = SyncFingerprint.ForConversation(id, normalizedTitle, messages, isProtected, protectionTicks);
        ...
        await _js.InvokeVoidAsync("idbPutMeta", new
        {
            ...
            protectionChangedTicks = protectionTicks
        });
    }
    ...
    public async Task UpdateIndexAfterSaveAsync(string id, List<ChatMessage> messages, List<LocalConvo> currentIndex, CancellationToken ct = default)
    {
        ...
        var isProtected = existing?.isPasswordProtected == true;
        var protectionTicks = existing?.protectionChangedTicks ?? 0;
        var contentFingerprint = SyncFingerprint.ForConversation(id, title, messages, isProtected, protectionTicks);
    ...
    public async Task SetPasswordProtectedAsync(string id, bool isProtected, long? protectionChangedTicks = null, CancellationToken ct = default)
    {
        ...
        var contentFingerprint = SyncFingerprint.ForConversation(id, title, messages, isProtected, ticks);
