...
public class WasmGuestDataMigrationService : IGuestDataMigrationService
{
        ...
        string guestKey)
    {
        ...
        // Stamp a change time when migrating a locked guest note so sync won't demote it from peers without ticks.
        var proticks = isProtected ? DateTime.UtcNow.Ticks : 0L;
        var fingerprint = SyncFingerprint.ForNote(targetId, title, entries, isProtected, proticks);
