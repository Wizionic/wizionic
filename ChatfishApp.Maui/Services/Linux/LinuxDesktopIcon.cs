...
/// <summary>
/// Installs the Chatfish icon into the user icon theme + a .desktop entry so the
...
/// freedesktop icon theme / matching .desktop file for app-id <c>com.chatfish.app</c>.
/// </remarks>
[UnsupportedOSPlatform("windows")]
[UnsupportedOSPlatform("OSX")]
internal static class LinuxDesktopIcon
{
	public const string ApplicationId = "com.chatfish.app";
	public const string ApplicationName = "Chatfish";
	...
	public static void Apply(Gtk.Window window)
	{
		try
		{
			...
			if (iconPath is null)
			{
				Console.WriteLine("[Desktop] no app icon found next to binary (appicon.png / chatfish.png)");
				return;
			}
	...
	private static string? ResolveIconPath()
	{
		...
		var candidates = new[]
		{
			...
			Path.Combine(baseDir, "chatfish-appicon.png"),
			Path.Combine(baseDir, "chatfish.png"),
			// Source-tree fallback when running from bin/Debug/net10.0
			Path.GetFullPath(Path.Combine(baseDir, "..", "..", "..", "Resources", "AppIcon", "chatfish.png")),
	...
	private static void EnsureUserIconTheme(string sourceIconPath)
	{
		...
		// Also as "Chatfish.png" for any manual Icon=Chatfish desktop entries.
	...
	{
		var baseDir = AppContext.BaseDirectory;
		var apphost = Path.Combine(baseDir, "Chatfish");
		...
		{
			// When launched via `dotnet Chatfish.dll`, ProcessPath is the host — prefer dll invocation.
			...
			{
				var dll = Path.Combine(baseDir, "Chatfish.dll");
				if (File.Exists(dll))
		...
		var dllOnly = Path.Combine(baseDir, "Chatfish.dll");
