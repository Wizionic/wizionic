...
| **Reliability and safety** | Locked core system prompt (not user-replaceable): human safety for physical devices; no help creating child sexual abuse material; no help planning or covering up a murder or planning harm to a person; human control (do not resist shutdown or persist yourself); only listed tools. Optional custom instructions, About you, and Memories are additive and cannot override those rules. Password-protected notebooks, chats, and albums are blocked from tools until unlocked in the UI. |
...
For this app that means: do not treat the model as the only control. Tool results (web pages, notes, MCP, connectors) are untrusted input. Home Assistant, the embedded browser, MCP, and scheduled Workflows can take real actions; they run only when you enabled them. Home Assistant tools execute when the model calls them (there is no extra confirm dialog). A local uncensored model can still be jailbroken — the app will not guarantee refusal. Defense in depth here is encryption, no server-side chat store, allow-listed tools, locked items, HTTPS on wizionic.com, and the report path — not a promise that every generated token is safe.
